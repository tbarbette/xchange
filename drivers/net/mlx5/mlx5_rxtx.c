		  ...
		  unsigned int olx)
{
	 ...
	 */
	csum = MLX5_TXOFF_CONFIG(CSUM) ? txq_ol_cksum_to_cs(loc->mbuf->ol_flags) : 0;
			...
			unsigned int olx)
{
	...
	loc->wqe_last = wqe;
	mlx5_tx_cseg_init(txq, wqe, 0, MLX5_OPCODE_TSO, olx);
			  ...
			  unsigned int olx)
{
	...
	wqe = txq->wqes + (txq->wqe_ci & txq->wqe_m);
	loc->wqe_last = wqe;
	mlx5_tx_cseg_init(txq, wqe, ds, MLX5_OPCODE_SEND, olx);
...
do_align:
	...
	wqe = txq->wqes + (txq->wqe_ci & txq->wqe_m);
	loc->wqe_last = wqe;
	mlx5_tx_cseg_init(txq, wqe, 0, MLX5_OPCODE_SEND, olx);
		   ...
		   unsigned int olx)
{
	...
	if (MLX5_TXOFF_CONFIG(CSUM) &&
	    txq_ol_cksum_to_cs(loc->mbuf->ol_flags) != es->cs_flags)
		return false;
